...
import type { Task, TaskStats } from '../domain/entities';
import type { TaskInput } from '@/lib/validators/task';

export const taskRepository = {
  async findByUserId(userId: string, status?: string): Promise<Task[]> {
    return prisma.task.findMany({
      where: { userId, ...(status && { status: status as Task['status'] }) },
      orderBy: [{ status: 'asc' }, { priority: 'desc' }, { dueDate: 'asc' }],
    }) as Promise<Task[]>;
  },

  async create(userId: string, data: TaskInput): Promise<Task> {
  ...
  async update(userId: string, id: string, data: Partial<TaskInput>): Promise<Task> {
  ...
  async assertOwnership(userId: string, id: string): Promise<void> {
    const found = await prisma.task.findFirst({ where: { id, userId }, select: { id: true } });
