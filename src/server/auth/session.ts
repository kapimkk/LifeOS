...
/**
 * Cria um par access/refresh, persiste o refresh hash no banco e seta cookies.
 */
...
/**
 * Revoga o refresh token informado e limpa cookies.
 */
...
/**
 * Renova a sessão usando o refresh token. Faz rotação (revoga antigo e emite novo).
 */
export async function refreshSession() {
  ...
  // Rotação: revoga atual e emite novo par.
  await prisma.refreshToken.update({
    where: { id: stored.id },
    data: { revokedAt: new Date() },
  });
...
/**
 * Obtém usuário atual a partir do access token; tenta refresh se expirado.
 */
